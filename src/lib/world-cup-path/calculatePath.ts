...
import { THIRD_PLACE_ALLOCATION_OPTIONS } from "../../data/world-cup-2026/third-place-options";
import {
...
export function calculateWorldCupPath(query: PathQuery): PathResult {
  ...
  const startingBranches = findStartingBranches(seed);

  if (startingBranches.length === 0) {
    throw new Error(`Unable to place seed ${seed} in the official Round of 32 bracket.`);
  }
  ...
  const rounds = ROUND_ORDER.map((round) => buildRoundSummary(round, branchesByRound.get(round) ?? [], team.id));
...
function findStartingBranches(seed: string): TargetBranch[] {
  return ROUND_OF_32.flatMap((match) => {
    if (nodeCanContainSeed(match.left, seed, match.right)) {
      return [{ matchId: match.matchId, targetNode: match.left, siblingNode: match.right }];
    }

    if (nodeCanContainSeed(match.right, seed, match.left)) {
...
function buildRoundSummary(round: KnockoutRound, branches: TargetBranch[], targetTeamId: Team["id"]): RoundOpponentSummary {
  const possibleOpponentTeamIds = unique(
    branches.flatMap((branch) => collectPossibleTeamIds(branch.siblingNode, branch.targetNode, targetTeamId)),
  ).sort();
...
function collectPossibleTeamIds(node: string, siblingNode: string | undefined, targetTeamId: Team["id"]): Team["id"][] {
  if (node.startsWith("W")) {
    const match = matchesById.get(Number(node.slice(1)));
    return match
      ? [
          ...collectPossibleTeamIds(match.left, match.right, targetTeamId),
          ...collectPossibleTeamIds(match.right, match.left, targetTeamId),
        ]
      : [];
  }

  if (/^[123][A-L]/.test(node)) {
    const seeds = expandSeedNode(node, siblingNode);
    return unique(
      seeds.flatMap((seed) => {
...
function expandSeedNode(node: string, siblingNode: string | undefined): string[] {
  if (/^[12][A-L]$/.test(node)) {
    return [node];
  }
  ...
  const assignments = winnerSeed
    ? THIRD_PLACE_ALLOCATION_OPTIONS.map((option) => option.assignments[winnerSeed]).filter(Boolean)
    : [];
...
function nodeCanContainSeed(node: string, seed: string, siblingNode: string | undefined): boolean {
  if (node === seed) {
    return true;
  }

  if (seed.startsWith("3") && node.startsWith("3")) {
    return expandSeedNode(node, siblingNode).includes(seed);
  }
