...
function createMarketSignal(signal: NewsImpactSignal, snapshots: TeamMarketSnapshot[]): MarketSignal {
  ...
  return {
    ...
    shortDescription: signal.oneLineSummary,
    explanation: `${signal.explanation} ${signal.disclaimer}`,
    confidence: signal.confidence === "medium" ? 72 : 58,
    dataPoints: [
      {
        label: "24h move",
        value: formatSignedPoints(signal.marketMove),
        tone: signal.marketMove >= 0 ? "positive" : "negative",
      },
      {
        label: "Articles",
        value: String(signal.relatedArticles.length),
        tone: "neutral",
      },
      {
        label: "Confidence",
