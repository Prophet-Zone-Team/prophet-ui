...
export const fundingSecondaryButtonClass = cn(
  "box-border flex h-[50px] w-full items-center justify-center rounded-[8px] border border-prophet-muted",
  "font-[Sora] text-base font-[400] leading-5 text-prophet-foreground transition-opacity hover:opacity-80",
  "disabled:cursor-not-allowed disabled:opacity-50"
);
      ...
      <header className="relative flex shrink-0 items-center justify-center px-5 pb-4 pt-5">
        {onBack ? (
          <button
            type="button"
            onClick={onBack}
            className="absolute left-5 top-5 inline-flex h-8 w-8 items-center justify-center border-0 bg-transparent p-0 text-prophet-foreground transition-colors hover:text-prophet-muted"
            aria-label={t("goBack")}
          >
            <ChevronLeft className="h-5 w-5" aria-hidden="true" />
          </button>
        ) : null}
        <h2 className="m-0 text-xl font-[500] leading-6 text-prophet-foreground">{title}</h2>
        <button
          type="button"
          onClick={onClose}
          className="absolute right-5 top-5 inline-flex h-8 w-8 items-center justify-center border-0 bg-transparent p-0 text-prophet-foreground transition-colors hover:text-prophet-muted"
