...
import { DepositStatusStep, formatStableflowStatusLabel } from "@/views/portfolio/deposit/deposit-status-step";
...
export function DepositDialog({ open, onClose }: DepositDialogProps) {
  ...
  const [depositMethod, setDepositMethod] = useState<DepositMethod>("connected");
  const [selectedToken, setSelectedToken] = useState<DepositSelectableToken | undefined>();
  ...
  const [polygonUsdcDestinationAssetId, setPolygonUsdcDestinationAssetId] = useState<string | undefined>();
  const [stableflowQuote, setStableflowQuote] = useState<QuoteResponse | undefined>();
  const [stableflowExecution, setStableflowExecution] = useState<StableflowDepositContext | undefined>();
  const [statusPhase, setStatusPhase] = useState<DepositStatusPhase>("bridging");
  const [bridgeStatusLabel, setBridgeStatusLabel] = useState<string | undefined>();
  const [convertStatusLabel, setConvertStatusLabel] = useState<string | undefined>();
  const [detectedUsdcAmount, setDetectedUsdcAmount] = useState<string | undefined>();
  const [detectedUsdceAmount, setDetectedUsdceAmount] = useState<string | undefined>();
  ...
  useEvmBalances({
    auto: true,
    enabled: open && !!session && depositMethod === "stableflow" && stableflowFundingTokens.length > 0,
  ...
  const balancesLoading =
    depositMethod === "stableflow" ? stableflowTokensLoading : connectedBalancesLoading;

  const selectableTokens = depositMethod === "stableflow" ? stableflowTokens : supportedAssets;
  ...
  const onContinueToAmount = async () => {
    ...
    try {
      ...
      if (depositMethod === "connected" && Big(latestBalanceUsd || 0).lt(selectedToken.minCheckoutUsd)) {
  ...
  const onContinueToConfirm = async () => {
    ...
    try {
      if (depositMethod === "stableflow" && isStableflowDepositToken(selectedToken) && session?.funderAddress) {
        const amountBaseUnits = Big(amount).times(10 ** selectedToken.decimals).toFixed(0, 0);
        ...
        const balancePayload = await pollFunderCollateralBalances((balances) => {
          ...
          if (mode === "wrap-only") {
            ...
            setDetectedUsdceAmount(
              BigInt(balances.usdce.balanceBaseUnits || "0") > 0n ? balances.usdce.balance : undefined,
            );
          }
        });
  ...
  const onConfirmDeposit = async () => {
    ...
    if (!isStableflowDepositToken(selectedToken) || !session.funderAddress || !polygonUsdcDestinationAssetId) {
  ...
  const onConfirmPendingConvert = async () => {
    if (!session?.walletAddress || !stableflowExecution || !funderCollateralBalances || !pendingConvertMode) {
      return;
    }
    ...
    try {
      const amountUsd = getPendingConvertAmountUsd(funderCollateralBalances, pendingConvertMode);
      ...
      try {
        ...
        console.warn("[deposit-dialog] syncCash after convert failed", syncError);
      }
        ...
        >
          ...
          {step === "confirm" && selectedToken ? (
            <DepositConfirmStep
              ...
              token={selectedToken}
              amount={amount}
              quoteMode={depositMethod === "stableflow" ? "stableflow" : "bridge"}
