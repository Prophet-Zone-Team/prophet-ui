...
export function SignalAllItem({ item, onSelect, className, teamFilter }: SignalAllItemProps) {
  ...
  return (
    <article
      className={cn(
        "relative flex w-full max-w-none flex-col gap-2 rounded-[12px] px-3 py-3 transition-colors hover:bg-prophet-hover md:h-[78px] md:flex-row md:items-center md:gap-4 md:px-5",
    ...
    >
      ...
      <div className="flex w-full shrink-0 items-center justify-between gap-2 md:w-[110px] md:flex-col md:items-start md:gap-1">
        <div className="flex w-full flex-1 items-center gap-2 md:gap-[8px] overflow-hidden">
          <TeamFlag
            ...
            fallback={false}
          />
          <span className="truncate text-[14px] font-[400] leading-[17px] text-prophet-foreground md:text-[16px] md:leading-[19px]">
            {teamDisplayName}
          </span>
        </div>
        <span className="shrink-0 whitespace-nowrap text-[12px] font-[400] leading-[14px] text-prophet-muted md:mt-1 md:text-[14px] md:leading-[17px]">
          {item.publishedAtFormatted}
        </span>
      </div>

      <div className="min-w-0 flex-1">
        <h3 className="m-0 line-clamp-2 text-base font-[500] leading-[19px] text-prophet-foreground md:truncate md:text-[18px] md:leading-[21px]">
          {item.headline}
        </h3>
        <p
          className="m-0 mt-1 line-clamp-2 text-[14px] font-[400] leading-[17px] text-prophet-muted md:mt-2 md:truncate md:whitespace-nowrap"
      ...
      <div className="flex shrink-0 items-center justify-between gap-2 md:justify-end md:gap-1">
        <span className="text-[12px] font-[400] leading-[14px] text-prophet-muted md:hidden">
          {t("impact")}
        </span>
